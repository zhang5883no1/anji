				...
				<tr align="center">
					...
					<td>
						<form:select path="userId" cssClass="userInput">
							...
							<core:forEach items="${queryRobotBasicInfoDto.customerlist}" var="users">
								<form:option value="${users.id }">${users.nickName}</form:option>
							</core:forEach>
						</form:select>
					</td>
					 ...
					 <ec:row>
					    ...
					    <ec:column alias="所属业务员" sortable="false">
					    	<select class="userInput _users_info" data-type="userId"> 
					    		<core:forEach items="${queryRobotBasicInfoDto.customerlist}" var="as">
				        			<core:if test="${as.id==bo.userId}">
				        				<core:forEach items="${queryRobotBasicInfoDto.customerlist}" var="bs">
				        					<core:if test="${as.roomNo==bs.roomNo }">
					        					<option class="${bs.id }" value="${bs.id }" <core:if test="${bs.id==bo.userId}">selected</core:if>>${bs.nickName }</option>
					        				</core:if>
					        			</core:forEach>
				        			</core:if>
				        		</core:forEach>
